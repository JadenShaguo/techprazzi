...
科技界的情报追踪者，科技狗仔，全网捕捉前沿信号。TechPrazzi 盯住真正有料的一线人物、产品动态、技术文章和播客，把碎片化公开信息整理成易于消化的摘要推送给你。

**Slogan：** Hunt every tech signal.（追捕每一条科技信号）

**理念：** 像科技狗仔一样追踪公开信号：少看转述和噪音，多抓一手动态、原始链接、产品细节和有信息密度的观点。
...
- 顶级 AI 播客新节目的精华摘要
- 26 位精选 AI 一线人物在 X/Twitter 上的关键观点和洞察
- AI 公司官方博客的完整文章（Anthropic Engineering、Claude Blog）
- TechPrazzi 信号评分：按发布、技术深度、agent、研究、市场信号等维度排序
- `TL;DR`、`SIGNAL HUNT`、`Why it matters`、`WATCH NEXT` 分层摘要
- 所有原始内容的链接
- 支持英文、中文或双语版本
...
2. 输入 "set up TechPrazzi" 或执行 `/techprazzi`
...
- "语言换成中文"
- "把摘要写得更简短一些"
- "重点关注 agents 和 developer tools"
- "把摘要深度改成 deep"
- "显示我当前的设置"

信息源列表（人物、播客和官方博客）由中心化统一管理和更新——你无需做任何操作即可获得最新的信息源。
...
TechPrazzi 会根据你的兴趣画像重排信号。配置保存在 `~/.techprazzi/config.json`：
...
支持的常用兴趣包括 `ai`、`agents`、`developer-tools`、`infrastructure`、`research`、`product`、`startups`。你也可以写自己的关键词。
...
2. 你的 agent 获取 feed——一次 HTTP 请求，不需要 API key；远程不可用时会回退到本地 feed
3. `prepare-digest.js` 为每条内容计算 `signalScore`、`signalTags`、`signalReasons`，并生成 `signalRadar`
4. 你的 agent 根据你的兴趣画像和 prompts 将原始内容重新混编为易消化的摘要
...
- 如果你使用 Telegram/邮件推送，相关 key 仅存储在本地 `~/.techprazzi/.env`
