...
// ============================================================================
// TechPrazzi — Central Feed Generator
// ============================================================================
  ...
  for (const blog of blogs) {
    ...
    try {
      ...
      const indexRes = await fetch(blog.indexUrl, {
        headers: { "User-Agent": "TechPrazzi/1.0 (feed aggregator)" },
      });
      if (!indexRes.ok) {
        errors.push(
...
async function main() {
  ...
  if (runTweets) {
    ...
    console.error(`  Found ${xContent.length} people with new tweets`);
    ...
    if (xContent.length === 0 && xErrors.length > 0) {
      throw new Error(
        `X feed failed: 0 people returned and ${xErrors.length} X API error(s) occurred`,
      );
    }
    ...
    console.error(
      `  feed-x.json: ${xContent.length} people, ${totalTweets} tweets`,
    );
  }
